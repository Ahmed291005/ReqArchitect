...
export const ContinueConversationInputSchema = z.object({
  conversationHistory: z.array(FlowMessageSchema).describe('The history of the conversation so far.'),
});
...
export const ContinueConversationOutputSchema = z.object({
  updatedRequirements: z.array(RequirementSchema).describe('The full, updated list of requirements in JSON format.'),
  followUpQuestion: z.string().describe('A relevant, open-ended question to elicit more requirements from the user.'),
});
